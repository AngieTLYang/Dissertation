...
# --- Paths ---
IMG_PATH = rf"C:\Users\88690\Desktop\Dissertation\image processing\imagesFromPhone\photo_2025-08-22_18-57-35.jpg"
OWL_PATH = rf"C:\Users\88690\Desktop\Dissertation\image processing\imagesFromPhone_labels\photo_2025-08-22_18-57-35.txt"
DOCLAYOUT_JSON = "detections.json"
...
# --- Load OWLv2 boxes (YOLO format, normalized) ---
...
pen_boxes = load_owl_boxes(OWL_PATH, IMG_W, IMG_H)
if len(pen_boxes) != 2:
    raise ValueError("Expected exactly 2 OWLv2 detections (two pens).")

# --- Determine left/right pen based on x-coordinate ---
if pen_boxes[0][0] < pen_boxes[1][0]:
    left_pen, right_pen = pen_boxes[0], pen_boxes[1]
else:
    left_pen, right_pen = pen_boxes[1], pen_boxes[0]

# --- Use closest points between the two pens ---
pen1_tip = (left_pen[2], (left_pen[1]+left_pen[3])/2)   # rightmost point of left pen
pen2_tip = (right_pen[0], (right_pen[1]+right_pen[3])/2) # leftmost point of right pen

# --- Rectangle enclosing the line with tolerance ---
TOL = 5  # pixels
...
# --- Load DocLayout detections ---
with open(DOCLAYOUT_JSON, "r", encoding="utf-8") as f:
    doclayout = json.load(f)
detections = doclayout["detections"]
...
for det in detections:
    ...
    # Check if box intersects pen-tip rectangle (with tolerance)
    rect_overlap = (x1 <= x_right + TOL and x2 >= x_left - TOL and
                    y1 <= y_bottom + TOL and y2 >= y_top - TOL)
    
    # Or if any corner of the box is near the line connecting pen tips
...
for (x1, y1, x2, y2) in pen_boxes:
    ...
    cv2.circle(img_cv, (int(x1), int(y1)), 5, (0,0,255), -1)  # optional tip marker

# Draw pen-tip rectangle (blue)
cv2.rectangle(img_cv, (int(x_left), int(y_top)), (int(x_right), int(y_bottom)), (255,0,0), 2)

# Draw line connecting pen tips (cyan)
cv2.line(img_cv, (int(pen1_tip[0]), int(pen1_tip[1])),
                 (int(pen2_tip[0]), int(pen2_tip[1])),
                 (0,255,255), 2)

# Draw filtered DocLayout boxes (green)
for det in filtered:
